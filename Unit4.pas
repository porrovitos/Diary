...
///////////СОЗДАНИЕ НОВОЙ ЗАПИСКИ\\\\\\\\\\
  ...
  if fileexists(username_unit2 + DateToStr(Date)+ '.txt') then
    begin
      ...
      Error_label.Caption:= 'Запись на такой дате уже существует.'+ #13#10+'Откройте запись и если надо - отредактируйте ее.';
    end
  else
    begin
...
buttonSelected := MessageDlg('Вы точно хотите удалить файл?',mtWarning, mbOKCancel, 0);
